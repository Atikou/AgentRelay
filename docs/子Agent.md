...
M5 引入**只读子 Agent**：独立上下文、受限工具权限、父 Agent 显式授权、结果汇总与超时控制。第一版不做写文件、不递归派生。

## 内置角色

| 角色 ID | 名称 | 权限 | 用途 |
| --- | --- | --- | --- |
| `code_review` | 代码审查 | `read` | 只读审查；任务含文件路径时会**预读**并走**单次审查**（免 JSON 协议）；否则 ReAct 循环，默认 **16** 次迭代、**180s** 超时 |
| `test_analyze` | 测试分析 | `read` | 只读分析测试/日志输出；默认 **8** 次迭代、**120s** 超时 |
...
返回 `summary` 字段为父 Agent 可消费的汇总文本。
...
├─ SubAgentCoordinator.ts # 并行派生 + 汇总
└─ types.ts
```

自检：`npm run test:subagent`（5 项）。
