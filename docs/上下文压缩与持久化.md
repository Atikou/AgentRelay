...
| `PromptBuilder` | 将 sections + 最近消息渲染为 `ChatMessage[]` |
...
`user_preferences` / `session_summary` / `task_state` / `project_context` / `relevant_memories` / `semantic_results` / `recent_tool_results` / `response_rules`
...
| GET | `/api/context/search?q=&scope=&scopeId=` | 记忆检索 |
