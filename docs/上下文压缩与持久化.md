...
| `PromptBuilder` | 将 sections + 最近消息渲染为 `ChatMessage[]`；历史 `tool` 记录在模型调用前降级为普通 `user` 文本，避免误触 OpenAI 原生 tool_calls 协议 |
...
持久化消息允许保留 `role=tool`，用于 `recent_tool_results`、`file_snippets`、审计和摘要；但这类消息属于 AgentRelay 自定义 ReAct 协议的历史结果。恢复历史会话继续发消息时，`renderedPrompt.finalMessages` 不会把它们作为 OpenAI/DeepSeek 原生 `tool` 消息发送，而是包成带说明的 `user` 历史文本，防止兼容端点因缺少前置 `assistant.tool_calls` 返回 400。
...
侧栏 **「上下文与记忆 (M6)」**：会话列表、新建会话、恢复预览、写入偏好记忆、关键词检索。
