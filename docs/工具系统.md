...
| `project_scan` | read | 否 | 轻量扫描项目结构、配置、源码根和重要文件 |
| `locate_relevant_files` | read | 否 | 根据任务目标生成 SearchPlan、排序候选文件并返回 primary/candidate |
| `context_pack` | read | 否 | 一次读取并摘要多个相关文件，减少连续 `read_file` |

修改已有文件时，Agent **应优先使用 `apply_patch`**，而非直接 `write_file` 整文件覆盖。

查找相关文件时，Agent **应优先使用 `project_scan` → `locate_relevant_files` → `context_pack`**，只有定位结果不足时再回退低层 `list_files` / `search_text` / `read_file`。
...
- Mock 工具**不会**进入 `createDefaultRegistry`，生产 `/api/tools` 默认仍只暴露 14 个内置工具。
