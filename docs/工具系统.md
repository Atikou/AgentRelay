...
查找相关文件时，Agent **应优先使用 `project_scan` → `symbol_search`（已知符号）→ `locate_relevant_files` → `context_pack`**，只有定位结果不足时再回退低层 `list_files` / `search_text` / `read_file`。
...
- Mock 工具**不会**进入 `createDefaultRegistry`，生产 `/api/tools` 默认仍只暴露 15 个内置工具。
