...
- **权限暂停持久化与恢复**：`PermissionRequestStore` 与 `PausedRunStore` 在服务实例中使用 `memory.db` 持久化；JIT 暂停后，即使浏览器刷新或服务重启，`GET /api/permission-requests/pending` 仍可查到待处理请求，`POST /api/runs/:runId/resume-permission` 会从暂停快照恢复同一段消息链。启动恢复遇到带暂停快照的 `running` Run 时，会把 Run 恢复为 `waiting_confirmation`；只有没有暂停快照的悬挂 `running` Run 才会标记为 `failed`。
...
- **消息角色边界**：AgentRelay 内部严格区分 `user` / `system` / `assistant` / `tool`。用户输入只能来自真实用户请求；续跑、工作流切换、通知、解析纠偏等运行态上下文统一写入 `system`；工具执行结果保留为 `role=tool`，不会伪装成用户消息。发送到模型前由 `src/model/messageBoundary.ts` 在 ModelClient 传输边界把内部 `tool` 渲染为带 `Source: tool` 来源说明的 `system` 文本，避免 OpenAI-compatible 服务把自定义 ReAct 工具结果误判为原生 `tool_calls` 协议。
