...
- **子 Agent 调度**：主 Agent 可通过 `dispatch_subagent` 派生子 Agent；`patch_worker` 须显式 `grantedPermissions` 含 `write`；派生深度受 `security.subagent.maxDispatchDepth` 约束（默认 1，不支持无限递归）。子 Agent 完成后写入 `NotificationQueue`（`source=subagent`）。
