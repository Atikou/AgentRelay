...
- **内部意图元信息**：`IntentRouter` 会先推断 `answer` / `plan` / `edit` / `run` / `verify` / `refactor` 等内部意图，再兼容映射到当前运行模式；`executionMeta.modeSource`、`executionMeta.intent`、`executionMeta.workflowType` 会返回给前端与审计链路。
