...
- **用户侧权限策略元信息**：`POST /api/agent` 可传 `permissionPolicy: "readOnly" | "confirmBeforeEdit" | "autoEdit" | "confirmBeforeRun" | "autoRun"`；未传时 `RunPolicyManager` 会按内部意图与 `autoConfirm` 保守推断，并在 `executionMeta.permissionPolicy` / `permissionPolicySource` 返回。当前阶段该字段用于可观测与后续 `PermissionGuard` 铺垫，实际工具放行仍受 `allowedPermissions`、确认门、Shell/Network/路径策略约束。
