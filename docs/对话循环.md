...
flowchart TD
    ...
    S --> PW{"计划/审阅 + 项目分析?"}
    PW -- "是" --> PRE["PlanWorkflow 只读预扫描<br/>project_scan → locate_relevant_files → context_pack"]
...
- **PlanWorkflow 预扫描**：计划/审阅模式且目标是项目/代码分析时，循环会在模型首轮前执行只读固定流程 `project_scan → locate_relevant_files → context_pack`，把项目结构、候选文件与上下文包作为额外用户消息注入，减少模型反复 `search_text` / `read_file` 探索导致的预算耗尽。
