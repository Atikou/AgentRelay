...
- **子 Agent 调度**：主 Agent 可通过 `dispatch_subagent` 派生子 Agent；`patch_worker` 须显式 `grantedPermissions` 含 `write`；派生深度受 `security.subagent.maxDispatchDepth` 约束（默认 1，不支持无限递归）。`dispatch_subagent` 会归一化常见模型错参（如 `task` 数组、空对象 `context`、角色别名和 `writeFilePickStrategy: null`），并在已收集 3 个成功子 Agent 结果后阻止继续派生，提示模型直接汇总 `final`。子 Agent 完成后写入 `NotificationQueue`（`source=subagent`）。
