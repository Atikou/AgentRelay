...
2. **（可选）模型存放到 D 盘**：设置用户环境变量 `OLLAMA_MODELS=D:\Ollama\models`，然后**重启 Ollama**（托盘图标退出再打开，或 `ollama serve`），再拉取模型。本项目已按此配置，权重约 1GB 落在 `D:\Ollama\models`。
...
5. 验证接入：
   ...
   `local-qwen35` 显示「可用」即成功。也可在测试台网页点「检测模型可用性」。
