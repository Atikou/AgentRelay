...
`AgentActionParser`、`AgentNotificationRenderer`、`AgentSystemPromptBuilder`、`AgentWorkflowCapabilityHint`、`AgentToolResultRenderer`、`AgentRunUsageSummary`、`AgentActivityTimelineFinalizer`、`SubagentDispatchGuard`。主循环只负责编排调用；
`AgentActionParser` 统一解析模型 JSON 动作并保留 `final.answer` 内代码块；`AgentNotificationRenderer` 统一把后台通知渲染为模型可见系统运行态消息；`AgentToolResultRenderer` 统一把 `AgentToolStep` 渲染为模型可见 observation；`AgentRunUsageSummary` 统一构建 `run_usage_summary` trace payload；`AgentActivityTimelineFinalizer` 统一根据 stopReason / completionGuard / 预算状态收尾 Timeline；`SubagentDispatchGuard` 统一处理 `dispatch_subagent` 三结果收敛、写/命令子任务父级权限与自动策略门控、失败提示渲染。
