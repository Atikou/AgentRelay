...
| 纯聊天旁路 | `CompanionService` 提供零工具、零权限、独立存储的虚拟聊天人格层 |
...
| 纯聊天 | `companion/` | 虚拟人格聊天、逐条落盘、摘要压缩、情感边界守卫 |
| 追踪 | `trace/`、`agent/timeline/` | JSONL 审计、公开 Activity |
| 生命周期 | `lifecycle/` | 用量统计、清理 |

**边界规则**：`model/` 不引用 agent/plan/tools；`model-router/` 不反向引用 `AgentLoop`，Agent chat 类型放在 `model-router/agent-chat-types.ts`；`companion/` 不引用 `AgentLoop`、`tools/` 或 `PermissionGuard`；工具副作用声明须诚实；知识层避免反向依赖执行层格式化逻辑。
...
| `SessionTaskManager` | `agent/task/SessionTaskManager.ts` | SQLite `session_task_contexts`（memory.db v24）+ 内存 |
...
映射单一来源：`agent/intentPatterns.ts`；公开 API/UI 剥离内部 mode 词汇见 `agent/RunPolicyVocabulary.ts`（`toPublicExecutionMeta`）。新代码优先 `intent` + `workflowType`，避免再引入第四套 mode 词汇。
...
`AgentLoop` 的 ReAct 动作解析、安全点通知回灌、系统提示、工作流能力提示、工具结果回灌、运行用量摘要、Activity Timeline 收尾与子 Agent 派发守卫已分别拆入
`AgentActionParser`、`AgentNotificationRenderer`、`AgentSystemPromptBuilder`、`AgentWorkflowCapabilityHint`、`AgentToolResultRenderer`、`AgentRunUsageSummary`、`AgentExecutionMetaBuilder`、`AgentPausedRunSnapshot`、`AgentToolStepBlockBuilder`、`AgentCapabilityEscalationOrchestrator`、`AgentToolStepPipeline`、`AgentToolActionRunner`、`AgentToolActivityTracker`、`AgentRunBootstrap`、`AgentReactLoopRunner`、`AgentRunFinalizer`、`AgentActivityTimelineFinalizer`、`SubagentDispatchGuard`。`AgentLoop.run()` 只负责状态重置与编排调用；
`AgentRunBootstrap` 负责会话初始化、工作流预扫描与 plan handoff 系统提示；`AgentReactLoopRunner` 承载 ReAct 主循环（模型轮次、parseAction、final/guard、工具后处理）；`AgentRunFinalizer` 统一 Run 收尾（上下文压缩、`executionMeta`、SessionTask、RunState、Timeline、`AgentRunResult` 组装）；`AgentToolActionRunner` 统一 `runToolAction` 实际执行。
...
| 存储 | 版本 | 内容 |
| --- | --- | --- |
| `memory.db` | v24 | 会话、记忆、计划、路由日志、权限暂停、session_task_contexts、workspace_grants、workspace_access_audit |
| `tools.db` | v2 | 工具备份、changeId、tool_logs、file_changes workspace metadata |
| `companion.db` | v1 | Companion 纯聊天会话、逐条消息、摘要；默认位于 `.agentrelay/companion/` |
...
- 主 Agent 派发门控：`SubagentDispatchGuard` 阻止 3 个成功结果后的继续派生，并要求写/命令型子任务同时满足父运行权限与自动策略。
...
- `PathPolicy` 在工具执行前判断路径属于主工作区、配置型工作区、已授权工作区还是未授权外部路径，覆盖 read/list/search/context_pack/project_scan/write/apply_patch/shell/git 等路径型工具；scope root 与目标路径比较前会经 `realpath`/canonicalize 规范化，避免 Windows 8.3 短路径与长路径被误判为跨工作区。
...
## 12. Companion 纯聊天旁路（MVP）

Companion 是 AgentRelay 之外的纯聊天产品面，不是新的 Agent mode，也不进入 workflow / capability escalation / PermissionGuard / ToolRegistry 链路。

- `CompanionService` 只依赖 `directChat` 与 `CompanionStorageManager`；默认人格由 `PersonaRuntime` 提供，`PromptComposer` 注入最近消息与摘要。
- HTTP 入口为 `/api/companion/*`：聊天、流式聊天、会话、消息、摘要、存储状态与向量状态。
- 默认存储目录是项目根下 `.agentrelay/companion/`，可通过 API/UI/后续桌面端显式传入 `storageRoot`；自定义目录只代表 Companion 应用数据目录，不授予读取该目录其他文件的能力。
- 用户消息收到后立即写入 `companion.db`；流式回复先保存 draft，完成后更新为 `completed`，异常中断保留 `interrupted` partial。
- 摘要压缩只影响上下文注入，不删除原始聊天记录；向量索引当前为 `enabled:false` 的接口占位，后续可接独立 namespace。
- `CompanionSafetyFilter`、`AttachmentRiskClassifier`、`RealityAnchorPolicy`、`VirtualIdentityBoundary`、`WarmthStyleGuard` 负责温暖但有边界的回复：可安慰、可疏导，但不强化依赖、占有、唯一支持或现实替代。
- `outputMode` 分为 `bounded` 与 `unrestricted`：`unrestricted` 用于本地模型无限制作答测试，不经 `CompanionSafetyFilter` 改写，消息正常写入 `companion.db` 并带 `companionMode=unrestricted`。
- 上下文隔离是单向的：`unrestricted` 可以读取正常模式历史；`bounded` 不会把 `unrestricted` 消息或带 `mode:unrestricted` 的摘要注入模型上下文。
- 测试台新增“纯聊天”入口，隐藏 mode/workflow/tool/permission 等 Agent 内部字段。

## 13. 已知架构债（见 TodoList）

- `AgentLoop` 仍保留工具管道上下文构建与 JIT pause 编排（可继续薄化）
- 计划模型 IO 仍使用 legacy `Plan` JSON（落盘必经 `PlanService` / `internalPlanFromLegacy`）
- UI 高级选项仍暴露 `mode` 调试（`?dev=1`）
- AI diff 持久化 trace 表待建
