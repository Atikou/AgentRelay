...
`AgentLoop` 的系统提示、工作流能力提示与子 Agent 派发守卫已分别拆入
`AgentSystemPromptBuilder`、`AgentWorkflowCapabilityHint`、`SubagentDispatchGuard`。主循环只负责编排调用；
`SubagentDispatchGuard` 统一处理 `dispatch_subagent` 三结果收敛、写/命令子任务父级权限与自动策略门控、失败提示渲染。
