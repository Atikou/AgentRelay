...
`GET /api/config` 另返回 `workspaces`（可选配置项 `config/*.json` → `workspaces[]`：`id` / `label` / `root`）与 `defaultWorkspaceKey`。测试台「＋ 新会话」会据此弹出工作区选择；`POST /api/context/sessions` 可传 `workspaceKey`（配置项）或 `workspaceRoot`（手动输入任意本机目录）绑定会话级工具沙箱根目录（创建后不可更改），Agent 执行时按会话解析 `workspaceRoot`。
...
| 上下文与记忆 | `/api/context/*` | M6 会话持久化、压缩与检索；`POST /api/context/sessions` 支持 `workspaceKey` / `workspaceRoot`；`POST .../purge` 隐私清除 |
