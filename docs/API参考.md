...
- `POST /api/plans/analyze`：生成并保存 `UserVisiblePlan`，只读、不可执行。
- `POST /api/plans/{userVisiblePlanId}/compile`：仅把已确认 Todo 编译成 `InternalTaskPlan` 草案，仍需审批后执行。
