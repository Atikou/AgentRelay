...
## 1.5 Companion 纯聊天请求（旁路）

`/api/companion/*` 是独立产品面，不进入 `Orchestrator.prepareAgentRun`，也不创建 Agent workflow、tool action、permissionRequest 或 ToolLedger。

```mermaid
sequenceDiagram
  participant U as 用户/测试台
  participant API as /api/companion/chat
  participant CS as CompanionService
  participant ST as CompanionStorage
  participant DC as directChat
  participant SF as CompanionSafetyFilter

  U->>API: message + sessionId? + storageRoot? + incognito? + outputMode?
  API->>CS: chat / chatStream
  CS->>ST: 保存 user message（非 incognito）
  CS->>ST: 读取最近消息 + summaries
  CS->>DC: 纯文本聊天（无 tool protocol）
  DC-->>CS: assistant content / token
  CS->>SF: 情感边界、现实锚定、虚拟身份检查
  CS->>ST: 保存 assistant draft/final（非 incognito）
  CS-->>U: content + storage + safety + summaryStatus
```

约束：

- `CompanionService` 只依赖 `directChat`、`PersonaRuntime`、`PromptComposer`、`CompanionStorageManager` 与安全过滤器。
- `storageRoot` 只是 Companion 应用数据目录；自定义目录不会变成工作区授权，也不会允许读取该目录内任意文件。
- `incognito:true` 只保留本轮内存上下文，不写长期消息、摘要或向量状态。
- `outputMode=unrestricted` 进入无限制作答测试模式：模型输出不经 `CompanionSafetyFilter` 改写，作为该模式下的正式消息写入存储，并标记 `companionMode=unrestricted`。
- 上下文隔离：`unrestricted` 可读取正常模式历史；`bounded` 生成时过滤 `unrestricted` 消息与带 `mode:unrestricted` 的摘要。
- SSE 流式回复先保存 assistant draft，完成后更新为 `completed`；异常中断保存 `interrupted` partial。
- 当用户要求改文件、运行命令、控制浏览器或应用时，纯聊天层只能自然说明“这需要切换到 Agent 执行”，不能自动调用 `/api/agent`。
...
flowchart TD
  ...
  PRE -->|无| MODEL
  MODEL --> PARSE{AgentActionParser 解析 JSON}
...
`AgentActionParser` 先去除 `<think>` / `<redacted_reasoning>` 噪声，再从模型输出中提取 `tool` / `final` 动作；解析前不剥离 Markdown 围栏，避免破坏 `final.answer` 内的代码块或 JSON 示例。

安全点消费的后台通知由 `AgentNotificationRenderer` 渲染为系统运行态消息后注入模型上下文；通知只作为当前任务的参考信号，不改变权限策略，也不打断既有工具链。

Run 收尾时由 `AgentRunUsageSummary` 构建 `run_usage_summary` trace payload，汇总模型轮次、token、费用、工具失败分类与 blocked 数量；`AgentLoop` 只负责把 payload 写入 trace。

Activity Timeline 收尾由 `AgentActivityTimelineFinalizer` 执行：根据 `stopReason`、`completionGuard` 与预算 ledger 把 Run 标记为完成、部分完成、等待授权、预算耗尽或用户取消；`AgentLoop` 只传入收尾所需状态。`executionMeta` 由 `AgentExecutionMetaBuilder` 组装（工作流产物、toolLedger、`userFacingLabel`、预算耗尽 enrich），主循环只收集 Run 上下文后调用。JIT 权限暂停时，`AgentPausedRunSnapshot` 负责收集 `runtimeState`、深拷贝对话/步骤/工作流产物并创建 `permissionRequest`；`AgentLoop` 仅调用 `buildPausedRunSnapshot` / `restorePausedRunRuntimeState` 薄封装。
...
`ToolRegistry` 统一解析 `outcomeClass/outcomeKind`；`AgentToolResultRenderer` 将 `AgentToolStep` 渲染为模型可见 observation；`FailedActionMemory` 记录重复观察失败并熔断；`ToolRecoveryWorkflow` 按 kind 注入恢复路线。Trace/Timeline 区分 `execution_error` 与 `observation_failure`。
...
| 模块 | 职责 |
| --- | --- |
| `run()` | `AgentRunBootstrap.bootstrapAgentRunSession`（会话/预扫描/handoff）→ `AgentReactLoopRunner.runAgentReactLoop`（ReAct 主循环） |
| `executeToolStep` | 主循环与 JIT resume 共用：`AgentToolStepPipeline.executeAgentToolStepPipeline`；`AgentLoop` 处理 JIT `pause` 与 `budget` 收尾 |
| `runToolAction` | `AgentToolActionRunner.runAgentToolAction`（Timeline / 路径审计 / 缓存 / 子 Agent 门控 / write gate / registry）；`AgentToolActivityTracker` 管理单次工具 Timeline 步骤 |
| `finishRun` | `AgentRunFinalizer.finalizeAgentRun`（上下文压缩、SessionTask、RunState、Timeline、`AgentRunResult`） |
| `EffectiveWorkflowContext` | `entry*` / `reconciled*` / `effective*` 贯通 WriteGate、PermissionGuard、SessionTask |
| `PausedRunSnapshot.runtimeState` | pause 时由 `AgentPausedRunSnapshot` 保存 escalation、预算分项、缓存、FailedActionMemory；resume 时恢复 |
...
| CapabilityEscalation | soft workflow 工具超出默认预期时升级 reconciled workflow，记录到 `executionMeta.capabilityEscalations`（编排见 `AgentCapabilityEscalationOrchestrator`） | 不阻断；交 PermissionGuard |
