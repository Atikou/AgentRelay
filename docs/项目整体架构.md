...
| 编排 | RunPolicyTypes | `agent-relay/src/agent/RunPolicyTypes.ts` | RunPolicy / 预算 / executionMeta 纯类型与 `parseRunModeValue`；供 Agent/Orchestrator 共享，避免策略管理器循环依赖 |
| 编排 | IntentRouter | `agent-relay/src/agent/IntentRouter.ts` | 模式推断（显式/文案/taskType）+ 联动 `WorkflowPlanner` |
...
└─ agent-relay/
   ...
   └─ src/
      ...
      ├─ agent/                   # AgentLoop / RunPolicyManager / RunPolicyTypes / BudgetManager / Planner
