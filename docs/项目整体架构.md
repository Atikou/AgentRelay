...
| 编排 | WorkflowExecutor | `agent-relay/src/agent/WorkflowExecutor.ts` | 预模型确定性工作流统一调度入口；当前挂接 `PlanWorkflow` 与 `RunVerifyWorkflow`，其中 edit/generate-file 会先只读定位并打包上下文，向 `AgentLoop` 返回新增步骤和模型上下文 |
