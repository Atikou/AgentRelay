...
| 编排 | EditVerificationWorkflow | `agent-relay/src/agent/EditVerificationWorkflow.ts` | edit/generate-file 的写入后验证阶段上下文；观察写入后的 read/diff/shell 等验证工具结果，注入 verification phase，并生成 `executionMeta.workflowVerifications` 审计记录 |
...
└─ agent-relay/
   ...
   └─ src/
      ...
      ├─ agent/                   # AgentLoop / IntentRouter / WorkflowRouter / WorkflowExecutor / PlanWorkflow / DebugAnalysisWorkflow / EditProposalWorkflow / EditExecutionWorkflow / EditAutoVerificationWorkflow / EditVerificationWorkflow / PlanReportWorkflow / PlanCompileWorkflow / TaskExecutionWorkflow / RunVerifyWorkflow / RunPolicyManager / RunPolicyTypes / BudgetManager / Planner
