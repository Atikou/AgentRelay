...
| 工具 | ToolRegistry + 本地工具 | `agent-relay/src/tools/` | 15 个内置工具 + ToolStorage 备份/日志；含 `symbol_search` 与相关文件定位工具 |
...
| 上下文 | ProjectIndex | `agent-relay/src/context/ProjectIndex.ts` | SQLite `project_files`/`project_symbols`/`project_imports`/`project_exports`；`project_scan` 增量索引、`locate` 复用 |
