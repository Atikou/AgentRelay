...
| 编排 | WorkflowExecutor | `agent-relay/src/agent/WorkflowExecutor.ts` | 预模型确定性工作流统一调度入口；当前挂接 `PlanWorkflow`、`EditProposalWorkflow` 与 `RunVerifyWorkflow`，向 `AgentLoop` 返回新增步骤和模型上下文 |
...
| 编排 | EditProposalWorkflow | `agent-relay/src/agent/EditProposalWorkflow.ts` | edit/generate-file 的写入前方案阶段；不执行工具，只注入 `targetFiles` / `changeSummary` / `permissionCheck` / `diffPlan` / `verificationPlan` 约束 |
