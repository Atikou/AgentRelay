...
| 编排 | Orchestrator | `agent-relay/src/orchestrator/` | 统一 Run：Agent/Task/Chat/Plan/Scheduler 创建 runId、写 Task、同步 task_steps/task_attempts、调执行器 |
| 策略 | policy | `agent-relay/src/policy/` | Workspace / Shell / Permission 策略（供工具与后台共用）；ShellPolicy 支持命令风险分级与 allow/deny 正则 |
| 编排 | AgentLoop | `agent-relay/src/agent/AgentLoop.ts` | 自主对话循环（M1）：模型按 ReAct JSON 协议决定工具调用，迭代至最终答案；写入 `agent_decision`、`agent_model_turn`、`run_usage_summary`，工具调用带 `toolCallId` |
| 编排 | RunPolicy | `agent-relay/src/agent/RunPolicy.ts` | Agent 运行模式、默认预算、只读/写入权限边界与 `executionMeta` 停止原因 |
| 编排 | PlanWorkflow | `agent-relay/src/agent/PlanWorkflow.ts` | 计划/审阅类项目分析前的只读预扫描：`project_scan` → `locate_relevant_files` → `context_pack`，降低主循环探索轮次 |
| 编排 | Planner | `agent-relay/src/agent/Planner.ts` | 调模型生成**结构化计划**并拆分子任务（目标/输入输出/验收/步骤元数据），只读 |
| 编排 | PlanService | `agent-relay/src/plan/` | **AgentStepPlan / UserVisiblePlan / InternalTaskPlan** 分离；UserVisiblePlan 持久化、编译；InternalTaskPlan 校验、审批、预览；Executor 仅认 planId+version |
...
| 工具 | ToolRegistry + 本地工具 | `agent-relay/src/tools/` | 14 个内置工具 + ToolStorage 备份/日志；含相关文件定位工具；`tool_audit` 携带 `toolCallId`；`mockTools.ts` 提供测试专用 mock 注册表 |
| 模型 | ModelRouter | `agent-relay/src/model/ModelRouter.ts` | 客户端级策略 + fallback（Agent/指定 client 时）；`sensitive` / `privacy-first` 仅本地；remote client 调用前脱敏敏感消息 |
| 模型 | SmartModelRouter + FallbackManager | `agent-relay/src/model-router/` | 规则路由 + `routerProfile` 等级 + 协作策略决策 + V2 模型升级 fallback；继承 `localOnly` 隐私约束 |
| 模型 | ModelOrchestrator | `agent-relay/src/model-orchestrator/` | `single_model` / `strong_model_direct` / `local_draft_remote_review` 执行 |
...
| 子 Agent | SubAgentCoordinator | `agent-relay/src/subagent/SubAgentCoordinator.ts` | 并行派生、结构化汇总、共同结论/冲突检测、parentTaskId 链路 |
...
flowchart LR
    ...
    E --> F["PlanCompiler → InternalTaskPlan draft"]
...
> 任务真实执行经 `Orchestrator.runTask` → `TaskRunner`（`dependsOn` DAG 波次并行）→ `ToolStepExecutor` → `ToolRegistry`；干跑使用 `DryRunExecutor`。Plan 步骤会同步到 `task_steps` / `task_step_dependencies`，每次执行尝试写入 `task_attempts`，步骤与聚合任务状态变化写入 `task_status_change` trace。请求体 `rollbackOnFailure: true` 时，失败/blocked 会经 `TaskRollback` 逆序调用 `rollback_change` 补偿文件变更。
...
└─ agent-relay/
   ...
   └─ src/
      ...
      ├─ orchestrator/            # Orchestrator + RunStore
      ├─ policy/                  # Workspace / Shell / Permission
