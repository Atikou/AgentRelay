...
| 编排 | WorkflowRouter | `agent-relay/src/agent/WorkflowRouter.ts` | P2 工作流路由骨架：从 `intent` 映射到 `workflowType`、执行器标识、只读/副作用类别 |
...
└─ agent-relay/
   ...
   └─ src/
      ...
      ├─ agent/                   # AgentLoop / IntentRouter / WorkflowRouter / IntentTypes / RunPolicyManager / RunPolicyTypes / BudgetManager / Planner
