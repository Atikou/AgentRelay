...
├─ docs/                          # 说明文档（被文档站自动渲染）
│  ├─ 项目整体架构.md             # 本文
│  ├─ API参考.md                  # REST 总览 + /api-docs 入口
│  ├─ 接入本地模型.md
│  └─ assets/                     # 截图
└─ agent-relay/
   ├─ config/                     # default / local-only / cloud 等 profile
   ├─ public/                     # 测试台前端 + openapi.json + api-docs.html
   ├─ scripts/                    # 截图等脚本
   ...
   └─ src/
      ...
      ├─ subagent/                # SubAgentRunner / SubAgentCoordinator (M5)
      ├─ context/                 # ContextManager / Restorer / Retriever / PromptBuilder (M6)
...
| 状态 | 说明 |
| --- | --- |
| ✅ 已完成 | 模型路由、Agent 模式、工具系统、M1 AgentLoop、M4 后台与通知、**M5 只读子 Agent**、**M6 上下文压缩与持久化**、**M7 安全与审计**（脱敏 trace + 工具审计 + 导出 API） |
| 🚧 进行中 / 下一步 | M7 安全审计、流式逐步推送、主 Agent 自动派生子 Agent |
| ⏳ 规划中 | 定时触发、子 Agent 写权限、通知去重/合并、安全审计增强 |
