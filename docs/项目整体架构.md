...
flowchart TD
    ...
    subgraph TOOLS["工具层 src/tools/"]
        ...
        FT["11 个本地工具<br/>patch/backup/git…"]
    end
    ...
    subgraph SUB["子 Agent 层 src/subagent/ (M5)"]
        ...
        SAC["SubAgentCoordinator<br/>并行 + 汇总"]
    end
...
| 策略 | policy | `agent-relay/src/policy/` | Workspace / Shell / Permission 策略（供工具与后台共用） |
...
| 工具 | ToolRegistry + 本地工具 | `agent-relay/src/tools/` | 11 个第一阶段工具 + ToolStorage 备份/日志 |
| 模型 | ModelRouter | `agent-relay/src/model/ModelRouter.ts` | 客户端级策略 + fallback（Agent/指定 client 时） |
| 模型 | SmartModelRouter | `agent-relay/src/model-router/` | 规则路由 + `routerProfile` 等级 + 协作策略决策 |
| 模型 | ModelOrchestrator | `agent-relay/src/model-orchestrator/` | `single_model` / `local_draft_remote_review` 执行 |
...
| 子 Agent | SubAgentCoordinator | `agent-relay/src/subagent/SubAgentCoordinator.ts` | 并行派生、结果汇总、parentTaskId 链路 |
...
| 上下文 | ContextRestorer | `agent-relay/src/context/ContextRestorer.ts` | 收集 session/摘要/记忆/项目/任务，返回 `ContextPackage` |
...
flowchart LR
    ...
    L --> N{"fallbackToPlanOnUncertainty?"}
    M --> N
    N -- 是且遇阻 --> O["Planner 修订计划 → modeFallback"]
```

> 任务真实执行经 `Orchestrator.runTask` → `TaskRunner`（`dependsOn` DAG 波次并行）→ `ToolStepExecutor` → `ToolRegistry`；干跑使用 `DryRunExecutor`。Plan 步骤会同步到 `task_steps` / `task_step_dependencies`，每次执行尝试写入 `task_attempts`。请求体 `rollbackOnFailure: true` 时，失败/blocked 会经 `TaskRollback` 逆序调用 `rollback_change` 补偿文件变更。
