...
| 编排 | WorkflowCorrectionWorkflow | `agent-relay/src/agent/WorkflowCorrectionWorkflow.ts` | 验证失败后的 correction/termination phase；按路径统计 attempt（默认最多 2 轮），生成 `executionMeta.workflowCorrections` |
...
| 编排 | WorkflowWriteGate | `agent-relay/src/agent/WorkflowWriteGate.ts` | edit/generate-file/debug 首次写入门控 |
| 编排 | EditWriteWorkflow | `agent-relay/src/agent/EditWriteWorkflow.ts` | edit/generate-file write phase；`executionMeta.workflowWritePhases` |
...
└─ agent-relay/
   ...
   └─ src/
      ...
      ├─ agent/                   # ... / ImplicitPlanWorkflow / WorkflowTaskState / WorkflowSessionSwitch / ...
