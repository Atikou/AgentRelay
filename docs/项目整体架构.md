...
| 策略 | policy | `agent-relay/src/policy/` | Workspace / Shell / Network / Permission / **ToolRiskAssessment**；`StructuredToolRisk` 统一高风险工具输出；`PermissionPolicy` 定义 project→mode→role→task→user 权限覆盖顺序 |
...
| 编排 | RunPolicyManager | `agent-relay/src/agent/RunPolicyManager.ts` | 模式推断、分项预算默认值、权限与 systemHint；配对 BudgetManager |
...
| 上下文 | ProjectIndex | `agent-relay/src/context/ProjectIndex.ts` | SQLite `project_files`/`project_symbols`；`project_scan` 增量索引、`locate_relevant_files` 复用 |
...
| 基础 | sqliteMigration | `agent-relay/src/storage/sqliteMigration.ts` | `schema_migrations` 审计表 + `PRAGMA user_version` 递增迁移；`memory.db` v7、`tools.db` v1 |
...
└─ agent-relay/
   ...
   └─ src/
      ...
      ├─ agent/                   # AgentLoop / RunPolicyManager / BudgetManager / Planner
