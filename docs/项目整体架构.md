...
| 编排 | AgentToolStep | `agent-relay/src/agent/toolStep.ts` | Agent 工具步骤共享类型；供 AgentLoop、PlanWorkflow、Orchestrator、子 Agent 复用，避免执行循环与预扫描模块互相导入 |
...
| 基础 | router-model-evaluator | `agent-relay/src/model-router/router-model-evaluator.ts` | V3 启发式路由评估；`unknown` 等不确定任务可覆盖，高风险不覆盖 |
