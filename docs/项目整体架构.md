...
| 编排 | EditProposalWorkflow | `agent-relay/src/agent/EditProposalWorkflow.ts` | edit/generate-file 的写入前方案阶段；不执行工具，只注入 `targetFiles` / `changeSummary` / `permissionCheck` / `diffPlan` / `verificationPlan` 约束，并生成 `executionMeta.workflowProposals` 可审计契约 |
