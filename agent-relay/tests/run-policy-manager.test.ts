...
test("inferMode 不再从计划关键词直接映射 plan，显式 mode 才覆盖", () => {
  assert.equal(
    defaultRunPolicyManager.inferMode({ message: "请进入计划模式，只读分析当前项目" }),
    "chat",
  );
  assert.equal(
    defaultRunPolicyManager.inferMode({
      requestedMode: "plan",
      forceMode: true,
...
test("answer/summarize/search 工作流即使显式自动策略也保持只读工具上限", () => {
  ...
  assert.equal(search.intent, "answer");
  assert.equal(search.legacyIntentHint, "search");
...
test("resolve 支持显式权限策略覆盖推断", () => {
  ...
  assert.equal(policy.intent, "answer");
  assert.equal(policy.legacyIntentHint, "verify");
...
test("resolve 根据意图与 autoConfirm 推断权限策略", () => {
  ...
  const run = defaultRunPolicyManager.resolve({
    requestedMode: "debug",
    forceMode: true,
    message: "运行测试验证结果",
  });
  assert.equal(run.permissionPolicy, "confirmBeforeRun");
  const autoRun = defaultRunPolicyManager.resolve({
    requestedMode: "debug",
    forceMode: true,
    message: "运行测试验证结果",
    autoConfirm: true,
