...
test("续跑完成 scan+locate 后不重复预扫描并写入 location", async () => {
  const goal = "请进入计划模式，只读分析当前项目模型路由模块并生成升级计划";
  const first = await makeOrchestrator(
    scriptedChat([
      '{"action":"tool","tool":"read_file","input":{"path":"package.json"},"thought":"尝试继续读取"}',
    ]),
  ).runAgent({
    message: goal,
    mode: "plan",
    budget: { maxToolCalls: 2, maxReadCalls: 20, maxModelTurns: 2 },
    persist: false,
  });
  ...
  const firstBody = first.body as {
    ...
    runState?: {
      ...
      location?: { visitedFiles: string[]; searchPlan?: { goal: string } };
    };
  };
  assert.equal(firstBody.reachedLimit, true);
  assert.ok(firstBody.runState);
  assert.deepEqual(firstBody.runState!.completedSteps, ["project_scan", "locate_relevant_files"]);
  assert.deepEqual(firstBody.runState!.pendingSteps, ["context_pack"]);
  assert.ok(firstBody.runState!.location);
  assert.ok(
    (firstBody.runState!.location!.visitedFiles?.length ?? 0) > 0 ||
      firstBody.runState!.location!.searchPlan?.goal,
  );

  const scanCountFirst = firstBody.steps.filter((s) => s.tool === "project_scan").length;
  assert.equal(scanCountFirst, 1);

  const resumed = await makeOrchestrator(
    scriptedChat(['{"action":"final","answer":"续跑后仅补 context_pack"}']),
  ).resumeAgent({
    runId: firstBody.runId,
    budget: { maxToolCalls: 10, maxReadCalls: 20, maxModelTurns: 2 },
  });
  ...
  assert.equal(resumedBody.steps.filter((s) => s.tool === "project_scan").length, scanCountFirst);
  assert.equal(resumedBody.steps.filter((s) => s.tool === "locate_relevant_files").length, 1);
  assert.ok(resumedBody.steps.some((s) => s.tool === "context_pack"));
  assert.match(resumedBody.answer, /续跑后仅补 context_pack/);
});
