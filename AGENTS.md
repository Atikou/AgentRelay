...
- M1 主对话循环（todolist 第 11/19 节）：`AgentLoop` 用可移植的 ReAct JSON 协议让模型自主决定工具调用，迭代到最终答案；含 `RunPolicy` 模式/分项运行预算、计划/审阅只读执行边界、`PlanWorkflow` 只读预扫描、权限/确认、预算耗尽部分收尾与 `executionMeta`。**默认模型选型经 `createAgentChatFn` → SmartModelRouter**（显式 `clientName` 仍走 ModelRouter）。接口 `POST /api/agent` 与 SSE `POST /api/agent/stream`（`onStep` 逐步推送；`streamTokens=true` 时 OpenAI 兼容模型可推送 `token` 事件），测试台「智能体」模式。
...
**未实现**（按里程碑推进）：V4+ AnswerEvaluator 运行时接入、RuntimeStats、并行投票、子 Agent 写权限/递归、多模态附件/OCR。

**路由覆盖面（2026-06-13）**：`/api/chat`、`Planner`、`/api/agent`、子 Agent 默认经 `SmartModelRouter`；显式 `clientName` 仍走 `ModelRouter`。

**V3 部分落地**：`RouterModelEvaluator` 启发式 V3 已接入 `DecisionEngine`（`source=evaluator`）；`AnswerEvaluator` 仍为 stub。
