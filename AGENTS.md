...
- M1 主对话循环（todolist 第 11/19 节）：`AgentLoop` 用可移植的 ReAct JSON 协议让模型自主决定工具调用，迭代到最终答案；含 `RunPolicy` 模式/分项运行预算、**`BudgetManager`** 分项硬限制与建议预算、计划/审阅只读执行边界、`PlanWorkflow` 只读预扫描、权限/确认、预算耗尽部分收尾与 `executionMeta`；**`RunStateStore`** 持久化 PlanWorkflow `pendingSteps` 并支持 `POST /api/agent/resume` 续跑；**`ToolResultLayers`** 工具结果 raw/model/user 三层 trace。**默认模型选型经 `createAgentChatFn` → SmartModelRouter**（显式 `clientName` 仍走 ModelRouter）。接口 `POST /api/agent` 与 SSE `POST /api/agent/stream`（`onStep` 逐步推送；`streamTokens=true` 时 OpenAI 兼容模型可推送 `token` 事件），测试台「智能体」模式。
