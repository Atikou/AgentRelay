...
- M5 子 Agent（只读 + 写权限 `patch_worker`）：`code_review` / `test_analyze` / `patch_worker`；`SubAgentRunner` + `SubAgentCoordinator`；主 Agent `dispatch_subagent`；完成通知入 `NotificationQueue`；`security.subagent.maxDispatchDepth` 有界递归（默认 1，不支持无限派生）；`/api/subagent/*`；测试台「子 Agent」面板。
